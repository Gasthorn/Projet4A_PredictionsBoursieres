...
def _compute_signal(df, ticker):
    empty = {'rec': 'SURVEILLER', 'signal_valid_to': '—', 'horizon_label': ''}
    if df.empty:
        return empty
...
def render_home_predictions(_, session):
        ...
        return html.Div(className="home-pred-login-wall", children=[
            ...
            html.Div(className="home-pred-login-btns", children=[
                html.A(
                    ...
                    href="/login",
                    className="home-pred-login-btn-primary",
                ),
                html.A(
                    [html.I(className="fas fa-user-plus"), "  Créer un compte"],
                    href="/signup",
                    className="home-pred-login-btn-ghost",
                ),
            ]),
        ])
    ...
    _rec_tooltip_icon = {'ACHETER': 'fas fa-circle-up', 'VENDRE': 'fas fa-circle-down', 'SURVEILLER': 'fas fa-circle-pause'}
    _rec_tooltip_color = {'ACHETER': '#00f0a0', 'VENDRE': '#ff4d6d', 'SURVEILLER': '#f0c040'}
    _rec_tooltip_title = {
    ...
    _rec_tooltip_body = {
        'ACHETER': "Notre IA a analysé des dizaines d'articles récents sur cette entreprise. Les nouvelles sont positives et les investisseurs réagissent bien. Si tu veux investir, c'est maintenant.",
        'VENDRE': "Les dernières actualités autour de cette entreprise sont mauvaises. Notre IA a repéré des signaux inquiétants — le cours risque de baisser. Mieux vaut ne pas investir maintenant.",
        'SURVEILLER': "Les signaux sont encore flous. Notre IA n'est pas assez sûre pour te donner un conseil clair. Patiente encore un peu et reviens bientôt — ça peut changer rapidement.",
    }
    ...
    for ticker, company in _COMPANIES.items():
        ...
        current_date = prices['current_date']

        tip_color = _rec_tooltip_color.get(rec, '#f0c040')

        cards.append(html.Div(className=f"home-pred-card {_rec_card.get(rec, 'home-pred-watch')}", children=[

            # Ticker + Recommandation
            ...
            html.Div(className="home-pred-tooltip", children=[
                html.Div(className="home-pred-tooltip-inner", children=[
                    html.I(
                        className=_rec_tooltip_icon.get(rec, 'fas fa-circle-info'),
                        style={"fontSize": "2rem", "color": tip_color, "marginBottom": "10px"},
                    ),
                    html.Div(_rec_tooltip_title.get(rec, ''), className="home-pred-tooltip-title",
                             style={"color": tip_color}),
                    html.Div(f"{company} — {ticker}", className="home-pred-tooltip-company"),
                    html.P(_rec_tooltip_body.get(rec, ''), className="home-pred-tooltip-body"),
